...
+(CCScene *) scene
{
	...
	HelloWorldLayer *layer = [[[HelloWorldLayer alloc] initWithColor:ccc4(255, 255, 255, 255)] autorelease];
...
-(id) initWithColor:(ccColor4B)color
{
    if ((self = [super initWithColor:color])) {
        ...
        showtip = (int)[userdata integerForKey:@"com.tidemobiles.numberseven.tip"];
        ...
        */
        ...
        CCLabelTTF* tc = [CCLabelTTF labelWithString:@"Life" fontName:@"Verdana" fontSize:16];
        tc.color = ccBLACK;
        ...
        timelab = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%d/10",life] fontName:@"Arial Rounded MT Bold" fontSize:24];
    ...
    if ((firstsp)&&(secondsp)&&(thirdsp)) {
        ...
        if (sum < 7) {
            if ((sound==0)||(sound==2))
                ...
                [[SimpleAudioEngine sharedEngine] playEffect:@"losthp.caf"];
            [self increaseHP:-1];
            score += 1;
...
-(void) increaseHP:(int)val
{
    ...
    if (val >0) {
        ...
        [timelab setString:[NSString stringWithFormat:@"%d/10",life]];
    }
    else {
        life += val;
        [timelab setString:[NSString stringWithFormat:@"%d/10",life]];
...
-(void) showGameOver
{
    ...
    if (score>0) {
        CCLOG(@"post score to game center:%d",score);
        [[GCHelper sharedInstance] reportEasyScore:score];
    }
...
{
    timeLeft -= 1;
    [timelab setString:[NSString stringWithFormat:@"%d/10",timeLeft]];
    if (timeLeft == 3) {
        if ((sound==0)||(sound==2))
...
-(void) showTip
{
    canPlayed = 0;
    CGSize wsize = [[CCDirector sharedDirector] winSize];
    CCSprite* tbg = [CCSprite spriteWithFile:@"tipbg.png"];
...
-(void) closeTip
{
    canPlayed = 1;
    NSUserDefaults* userdata = [NSUserDefaults standardUserDefaults];
    [userdata setInteger:1 forKey:@"com.tidemobiles.numberseven.tip"];
